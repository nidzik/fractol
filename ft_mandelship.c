...
/*   Updated: 2015/05/07 16:46:05 by lebijuu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (e.x < e.image_x)
	{
		...
		while (e.y < e.image_y)
		{
							...
							e.ite_max) || e.i == 0)
			{
				...
				be->data[e.y * be->size_line + 4 * e.x + 0] = mlx_get_color_value(be->mlx,  e.i * 0x000099 / e.ite_max);

				be->data[e.y * be->size_line + 4 * e.x + 1] =mlx_get_color_value(be->mlx,  e.i * 0x0000bb / e.ite_max);
				be->data[e.y * be->size_line + 4 * e.x + 2] = mlx_get_color_value(be->mlx,  e.i * 0x0000bb / e.ite_max);
