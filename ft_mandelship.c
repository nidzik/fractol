...
void ft_draw_mandelship(t_benv *be)
{
	...
	be->r * mlx_get_color_value(be->mlx,  e.i * 0x000099 / e.ite_max);
				be->data[e.y * be->size_line + 4 * e.x + 1] =
	be->g * mlx_get_color_value(be->mlx,  e.i * 0x0000bb / e.ite_max);
				be->data[e.y * be->size_line + 4 * e.x + 2] =
	be->b * mlx_get_color_value(be->mlx,  e.i * 0x0000bb / e.ite_max);
			e.y += 1;
		}
		e.x += 1;
	} 
